...
def start():
    ...
    if t.StartTime is not None:
        messagebox.showerror('error',"A timer is still running, please reset first!")
    
    t.start()
    txttimestart.set(t.StartTime.strftime("%m/%d/%Y, %H:%M:%S"))
    btstop.state(["!disabled"])
    btstart.state(["disabled"])
    #lbl_starttime.config(text=t.StartTime)
    
def stop():
    #global t
    elapsetime = t.stop()
    #lbl_stoptime.config(text=t.StopTime)
    ...
    str_entry = f"{txttimestart.get()} - {txttimestop.get()}: {elapsedtime_inmin} {strdescription}\n"
...
lbl_starttime = Label(root, font = ('calibri', 14, 'bold'), foreground = 'black',textvariable=txttimestart)
...
lbl_stoptime = Label(root, font = ('calibri', 14, 'bold'), foreground = 'black',textvariable=txttimestop)
