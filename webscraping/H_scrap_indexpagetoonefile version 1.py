...
def save_chapter_content_tofile(url, chaptertitle,savefilepath):
    ...
    content = chapterpage_soup.find_all("div", {"class":"tjc-cot"}) #change this name based on web.  
...
chapter_list = indexpage_soup.find_all("li",{"itemprop":"name"})
...
for chapter in chapter_list:
    ...
    #第 \u7b2c 回\u56de
    chinese_numberchar_unicode = ['\u4e00','\u5341','\u4e09','\u56db','\u4e94','\u516d','\u4e03','\u516b','\u4e5d','\u5341'] #chinese char,一...十
    if (chapter.get_text()[0] == chr(ord('\u7b2c'))): #check whether the first chinese char is 第
    #if (chapter.get_text()[1] in chinese_numberchar_unicode): #Change index to chapter.get_text()[1] if chapter title is  第一...
        print(f'{base_url}{chapter.a.get("href")} - {chapter.a.get_text()}')
        newurl = base_url + chapter.a.get("href")
