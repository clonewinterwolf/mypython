...
def save_chapter_content_tofile(url, chaptertitle,savefilepath):
    ...
    content = chapterpage_soup.find_all("div", {"id":"BookText"}) #change this name based on web.  
...
base_url = "https://www.wuxia8.com"
index_url = "https://www.wuxia8.com/wuxia/437/" #the url of index page containing url of all chapters 
...
#change below to match chatper start
...
#link_list[chapter_startindex:chapter_endindex]
chinese_numberchar_unicode = ['\u4e00','\u4e8c','\u4e09','\u56db','\u4e94','\u516d','\u4e03','\u516b','\u4e5d','\u5341']#chinese char,一...十
for index in range(0, totalrow):
    #if (chapter.get_text()[0] == chr(ord('\u7b2c'))): #check whether the first chinese char is 第
    for j in range(0, totalcol):
        ...
        if ((chapter_index < chapter_endindex) and (chapter_list[chapter_index].get_text()[1] in chinese_numberchar_unicode)): #Change index to chapter.get_text()[1] if chapter title is  第一...
            ...
            save_chapter_content_tofile(newurl, chapter_list[chapter_index].a.get_text(), book_filepath)
        else:
            print("skipped")
